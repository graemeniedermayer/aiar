...
export const basic_rigid_body = (() => {
  ...
  class BasicRigidBody extends entity.Component {
    ...
    InitEntity() {
      ...
      if(globals.debugging){
        const geometry = new THREE.CylinderGeometry( this.params_.size, this.params_.size, 0.2, 12 );
  ...
  class FloorRigidBody extends entity.Component {
    ...
    InitEntity() {
      ...
      if(globals.debugging){
        const geometry = new THREE.BoxGeometry( this.params_.size.x, this.params_.size.y, this.params_.size.z);
        const material = new THREE.MeshBasicMaterial( {color: 0x0000ff} );
        this.debug_ = new THREE.Mesh(geometry, material);
        this.debug_.position.copy(pos)
        this.debug_.quaternion.copy(quat)
  ...
  class ArrowRigidBodySystem extends entity.Component {
    ...
    InitArrow(id, params){
      ...
      if(globals.debugging){
        const geometry = new THREE.CylinderGeometry( 0.01, 0.01, 0.1, 12);
        const material = new THREE.MeshBasicMaterial( {color: 0xff0000} );
        let debug_arrow = new THREE.Mesh(geometry, material);
