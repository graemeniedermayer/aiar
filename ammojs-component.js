  ...
  class AmmoJSController extends entity.Component {
    ...
    StepSimulation(timeElapsedS) {
      ...
      if(player){
        ...
        if(globals.debugging == true){//bad style
        ...
        for(let k in this.projectiles_){
          ...
          e.position.copy(new THREE.Vector3(pos.x(), pos.y(), pos.z()) );
          e.quaternion.copy(new THREE.Quaternion(quat.x(), quat.y(), quat.z(), quat.w()) ); 
      ...
      for (let k in this.items_) {
        ...
        let pos = tmpTransform.getOrigin()
        let quat = tmpTransform.getRotation()
        e.SetPosition(new THREE.Vector3(pos.x(), pos.y(), pos.z()) );
        e.SetQuaternion(new THREE.Quaternion(quat.x(), quat.y(), quat.z(), quat.w()) ); 
      }
    }
